...
**Phase 2+ Architecture**: Lua connects directly to talker_service (port 5557) via WebSocket. TTS audio is generated by talker_service and sent as `tts.audio` messages. STT (speech-to-text) runs inside talker_service; audio is streamed from Lua as base64 Opus frames (native DLL `talker_audio.dll` handles microphone capture, Opus encoding, and VAD).
...
```
Lua (Game)                         talker_service
    │                                   │
    │  WS (5557)  ◄──────────────────► │
    │  JSON {t,p,r,ts}                 │  /ws
    │  game.event, dialogue, etc.       │
    │                                   │
    │  talker_audio.dll                 │
    │  (FFI: capture+opus)              │
    │  ──── mic.audio.chunk ───────────►│  STT
