...
# Faction name mappings for description placeholders
_FACTION_PLACEHOLDERS: dict[str, str] = {
    "stalker": "Loners",
    "dolg": "Duty",
    "freedom": "Freedom",
    "bandit": "Bandits",
...
def format_description(text: str) -> str:
    """Resolve faction placeholders in description text.
    
    Replaces %faction_name% patterns with human-readable faction names.
    
    Args:
        text: Description text with placeholders like %stalker%, %dolg%
    ...
    def replace_placeholder(match: re.Match) -> str:
        ...
        return _FACTION_PLACEHOLDERS.get(faction_id, faction_id)
...
LOCATION_DESCRIPTIONS: dict[str, str] = {
    ...
    "k02_trucks_cemetery": "Trucks Cemetery is a vast scrapyard to the east of Rostok full of irradiated vehicles from the 1986 disaster. Both %bandit% and dangerous mutants are often found here.",
    ...
    "l02_garbage": "Garbage (an area connecting the Cordon to Rostok) is an area where radioactive trash heaps and broken machinery from the 1986 disaster was dumped. It houses Butcher's shop in an old train depot, and its south-central location means roaming %stalker% and %bandit% are common.",
    ...
    "zaton": "Zaton (located west of Pripyat and north of Jupiter) is a swamp-like area formed from a drained branch of the Pripyat River, housing the remains of numerous boats and barges in the drained riverbed. Beard's %stalker% settlement housed in the stranded remains of the tanker 'Skadovsk' is the northernmost safe haven for %stalker% in the Zone.",
...
def get_location_description(technical_id: str) -> str:
    """Get detailed description for a location.
    
    Resolves faction placeholders to human-readable names.
    
    Args:
        technical_id: Technical location ID (e.g., 'l01_escape')
        
    Returns:
        Description with placeholders resolved, or empty string if not found
    """
    if not technical_id:
        return ""
    
    # Try exact match first
    description = LOCATION_DESCRIPTIONS.get(technical_id)
    if description is None:
        # Try lowercase
        description = LOCATION_DESCRIPTIONS.get(technical_id.lower())
