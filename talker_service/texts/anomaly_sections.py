"""Anomaly section name mappings for STALKER zones.

Maps technical anomaly section IDs (e.g., 'zone_vortex') to human-readable descriptions
(e.g., 'a Vortex anomaly (gravitational whirlwind pulling objects inward).').

Mirrors bin/lua/domain/data/anomaly_sections.lua — keep in sync.
"""
...
def describe_anomaly_section(section: str) -> str:
    """Resolve a technical anomaly section ID to its human-readable description.

    Args:
        section: Technical section name (e.g., 'zone_vortex'), may be empty.

    Returns:
        Human-readable description string, or the raw section name if not found.
    """
    if not section:
        return "an anomaly"
