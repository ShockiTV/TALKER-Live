...
- **AND** scanning SHALL stop after the 5th match
...
### Requirement: $ref resolver

The filter engine SHALL provide a `resolve_refs(tbl, results_map)` function that recursively walks a table, replacing `"$ref:..."` strings with their resolved values from a results map keyed by query ID.

#### Scenario: $ref string resolved from results map
- **WHEN** value is `"$ref:mem.last_update_time_ms"` and results map has `mem.data.last_update_time_ms = 50000`
- **THEN** the value SHALL be replaced with `50000`

#### Scenario: $ref to unknown query returns error
- **WHEN** value is `"$ref:unknown.field"` and results map has no key "unknown"
- **THEN** an error string SHALL be returned (`"$ref: 'unknown' not yet resolved"`)

#### Scenario: $ref to failed query returns error
- **WHEN** referenced query has `ok: false`
- **THEN** an error string SHALL be returned (`"$ref: 'id' resolved to error"`)

#### Scenario: $ref resolves recursively in nested table
- **WHEN** `$ref` strings appear at arbitrary depth in a nested filter document
- **THEN** all occurrences SHALL be resolved in a single pass
