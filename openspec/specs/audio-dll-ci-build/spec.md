...
The build SHALL export all 14 `ta_*` functions with C linkage (`extern "C"` / `__declspec(dllexport)`). No C++ name mangling SHALL be present in the export table.

#### Scenario: All symbols exported
- **WHEN** `dumpbin /exports talker_audio.dll` is run
- **THEN** all 14 `ta_*` function names appear unmangled
