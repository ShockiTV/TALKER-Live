...
Defines the background generation subsystem: before speaker selection, candidates without backgrounds are identified, character info is fetched, and a dedicated one-shot LLM conversation generates JSON backgrounds that are persisted via state mutations.
