...
Defines the Lua-side TTS slot management system for in-engine NPC audio playback. Manages a pool of pre-deployed OGG slot files, round-robin allocation, binary writing, and 3D spatial audio playback on NPC game objects with position tracking.

## Requirements

### Requirement: Silent OGG slot files shipped with mod
The mod SHALL ship 200 silent OGG Vorbis files at `gamedata/sounds/characters_voice/talker_tts/slot_1.ogg` through `slot_200.ogg`. Each file SHALL be a valid OGG Vorbis containing silence (minimum duration to be accepted by X-Ray). These files exist so the X-Ray engine indexes them at startup.
