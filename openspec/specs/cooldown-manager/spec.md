...
Generic CooldownManager class for Lua trigger scripts. Eliminates duplicated cooldown timer logic across `talker_trigger_death`, `injury`, `artifact`, `anomalies`, and `task` scripts. Supports named timer slots, optional anti-spam layer, and configurable on-cooldown behavior.
