...
Defines the TTS and microphone topics exchanged between Lua and mic_python over the mic WebSocket channel.
...
- **AND** mic_python is ready to receive `mic.start`, `mic.stop`, `tts.speak` frames
...
- `mic.stop` → cancel current recording session
