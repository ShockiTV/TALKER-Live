...
The `memory_store` module SHALL store memory per character as a structured table with five fields: `events` (list), `summaries` (list), `digests` (list), `cores` (list), and `background` (table or nil). Each list item SHALL have a monotonically increasing `seq` number that is never reused within a character's memory.
...
The `summaries`, `digests`, and `cores` tiers SHALL store compressed memory objects with fields: `seq` (number), `tier` (string), `start_ts` (number), `end_ts` (number), `text` (string), and `source_count` (number).
...
Each tier SHALL have a configurable cap. When an append would exceed the cap, the oldest items (lowest `seq`) SHALL be evicted before the append. Default caps: events=100, summaries=10, digests=5, cores=5.
