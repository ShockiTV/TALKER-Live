...
Pure Lua data tables extracted from `talker_game_queries.script` into testable `bin/lua/domain/data/` modules. Contains static lookup tables for unique NPCs, mutant names, and character ranks/reputation — no engine dependencies.
...
The system SHALL provide a function (in `domain/data/ranks.lua`) to format character information for event descriptions.
