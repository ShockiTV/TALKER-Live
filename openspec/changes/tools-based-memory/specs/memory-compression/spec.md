...
**Reason**: Replaced by four-tier compaction cascade (events→summary→digest→core). The old three tiers (recent events, mid-term summary, long-term narrative) are superseded.
...
**Migration**: Compaction triggers when any tier exceeds its cap (events: 100, summary: 10, digest: 5, core: 5). Compaction logic lives in Python, not Lua.

### Bootstrap compression for new memories
**Reason**: Replaced by compaction cascade. There is no separate bootstrap path — the same events→summary compaction handles first compression.
...
**Migration**: Each tier compacts to the next: events→summary→digest→core. Each step is a separate LLM call.
