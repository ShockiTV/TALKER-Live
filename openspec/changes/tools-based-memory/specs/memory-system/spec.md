...
The Lua `memory_store` SHALL store per-NPC structured memory with five fields: `events` (list of structured events), `summary` (list of compressed memories), `digest` (list of compressed memories), `core` (list of compressed memories), and `background` (structured table or nil). It SHALL provide a unified DSL with `append`, `delete`, `set`, `update`, `query` operations. It SHALL also maintain a `global_event_buffer` for emission/psy-storm backfill.
...
### Requirement: Memory Store Responsibilities
**Reason**: Replaced by four-tier-memory-store spec. The flat `{narrative, last_update_time_ms}` storage model is fully superseded by per-NPC structured storage with events/summary/digest/core tiers.
