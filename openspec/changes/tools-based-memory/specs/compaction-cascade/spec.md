...
- **WHEN** a character's summary tier has more than the summary cap (10)
...
- **WHEN** a character's digest tier has more than the digest cap (5)
- **THEN** the 2 oldest digests SHALL be read and merged into a single core entry

#### Scenario: Cores self-compact
- **WHEN** a character's core tier has more than the core cap (5)
- **THEN** the 2 oldest cores SHALL be merged into a single core entry
- **AND** the core tier remains the terminal tier (no further compaction)
...
- **THEN** only events→summary compaction SHALL run for that character

#### Scenario: Multi-tier cascade
- **WHEN** compacting events to a summary causes the summary tier to exceed its cap
- **THEN** summary→digest compaction SHALL also run in the same cycle
