...
The following files SHALL exist:
...
- **THEN** each of the 15 EventTypes SHALL have a corresponding `test_<type>_lifecycle.py` file
- **AND** each file SHALL contain at least one `@pytest.mark.asyncio` test
...
| ACTION | Various action text values |
