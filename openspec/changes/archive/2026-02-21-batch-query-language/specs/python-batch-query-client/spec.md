...
During the migration period, existing `query_*` methods on `StateQueryClient` (query_memories, query_character, query_world_context, etc.) SHALL continue to function. They MAY be deprecated with warnings but SHALL NOT be removed until all callers are migrated.

#### Scenario: Old query_memories still works
- **WHEN** `state_client.query_memories("123")` is called during migration
- **THEN** it SHALL function identically to before the change

#### Scenario: Deprecation warning on old methods
- **WHEN** an old `query_*` method is called
