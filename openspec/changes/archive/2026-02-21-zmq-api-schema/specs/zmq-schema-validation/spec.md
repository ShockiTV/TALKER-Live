...
The system SHALL provide a `schema_compiler.py` module in `talker_service/tests/e2e/` that loads `docs/zmq-api.yaml` and produces JSON Schema dicts.
...
- **WHEN** `compile_schema("docs/zmq-api.yaml")` is called
...
- Each entry in `expected.zmq_published[].payload` against the schema for that entry's `topic`
