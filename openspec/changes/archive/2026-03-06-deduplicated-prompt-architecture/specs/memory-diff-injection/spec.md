...
**Reason**: The `last_injected_timestamp` mechanism is replaced by the `DeduplicationTracker` which tracks individual (char_id, start_ts) pairs instead of a high-water-mark timestamp.

**Migration**: Memory injection tracking is now set-based via the dedup tracker. There is no single "latest timestamp" concept — each memory item is independently tracked.
