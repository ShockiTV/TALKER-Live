...
- **THEN** only events with `ts >= 300` SHALL be returned
...
### Requirement: Seq numbers are monotonically increasing (per-character)

**Reason**: Replaced by globally unique `ts` from `unique_ts()`. Per-character `seq` counters are no longer needed since `ts` provides both ordering and identity.

**Migration**: Existing `seq` fields are replaced with `ts` fields during v3→v4 save migration. The `next_seq` per-character counter is removed from persisted state.
