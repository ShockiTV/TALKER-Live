...
- **WHEN** a scenario file contains `expected.zmq_published` but omits `expected.http_requests`
- **THEN** the harness SHALL assert `zmq_published` deeply
- **AND** SHALL NOT assert HTTP calls (no failure for missing `http_requests` key)
...
- **WHEN** `assert_scenario` is called with `expected.http_requests`
