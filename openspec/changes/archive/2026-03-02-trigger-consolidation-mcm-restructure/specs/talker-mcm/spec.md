...
### Requirement: base_dialogue_chance global setting

**Reason**: Replaced by per-trigger `chance` integer inputs (0–100) under each trigger's MCM subsection. Every trigger now has its own chance setting.
**Migration**: Users with custom `base_dialogue_chance` values should set individual trigger `chance` values to their preferred percentages (multiply old float by 100).
