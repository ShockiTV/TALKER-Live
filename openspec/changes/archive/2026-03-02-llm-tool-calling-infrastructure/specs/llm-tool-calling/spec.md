...
The `OpenAIClient` SHALL implement `complete_with_tools()` by adding `tools` to the request body and parsing `tool_calls` from `choices[0].message`. It SHALL reuse the existing retry logic from `complete()`.

#### Scenario: OpenAI tool call response parsed
- **WHEN** OpenAI returns `choices[0].message.tool_calls` array
- **THEN** each tool call SHALL be parsed into a `ToolCall` with `id`, `name` (from `function.name`), and `arguments` (from JSON-parsed `function.arguments`)

#### Scenario: OpenAI text response returned
- **WHEN** OpenAI returns `choices[0].message.content` with no `tool_calls`
- **THEN** `LLMToolResponse(text=content, tool_calls=[])` SHALL be returned

#### Scenario: OpenAI rate limit retried
- **WHEN** OpenAI returns 429 during a `complete_with_tools` call
