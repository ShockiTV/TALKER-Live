...
function memory_store:load_save_data(saved_data)
    ...
    for id, data in pairs(saved_data) do
        ...
        if #data > 0 or (next(data) == nil and type(data) == "table") then 
            ...
            -- Migrate: Concatenate all content into the narrative.
            ...
            if #data >= transformations.COMPRESSION_THRESHOLD then
                 ...
                 -- We do NOT concatenate. We leave narrative empty and last_update_time at 0 (or undefined).
                ...
                table.sort(data, function(a, b) return a.game_time_ms < b.game_time_ms end)
...
function memory_store:get_new_events(character_id)
    ...
    -- Optimization: Use event_store:get_events_since if available, but for now filtering manually is safe
    for i = 1, #events do
        local event = events[i]
        if Event.was_witnessed_by(event, character_id) and event.game_time_ms > last_update_time then
            table.insert(uncompressed_memories, event)
        end
    end
    
    table.sort(uncompressed_memories, function(a, b) return a.game_time_ms < b.game_time_ms end)
    return uncompressed_memories
end

-- for the input speaker id, retrieves all new memories and the latest compressed memory
-- Legacy/helper support if anything calls this, we should use get_memory_context now
...
-- Deprecated but keeping signature for safety during refactor if needed, though get_memory_context is preferred
