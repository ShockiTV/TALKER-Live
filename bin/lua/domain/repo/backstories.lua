...
local function get_random_faction_backstory(faction)
    ...
    return queries.load_random_xml("stories_" .. tostring(faction))
end
...
local function set_random_backstory(character)
    ...
    local backstory = get_random_faction_backstory(character.faction)
...
function M.load_save_data(saved_character_backstories)
    if RESET_BACKSTORIES_ON_LOAD then
