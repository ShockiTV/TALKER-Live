-- infra/service/channel.lua
-- Service communication channel over WebSocket.
-- State machine: DISCONNECTED → CONNECTING → CONNECTED → RECONNECTING
-- Handles outbound queue, exponential backoff, request/response correlation,
-- topic handlers, and tick-based message drain.
--
-- Fully testable via ws_client socket factory injection and codec substitution.
...
local _pending          = {}   -- r → callback
local _on_reconnect     = nil  -- callback
local _was_connected    = false  -- tracks if we've been connected before (for on_reconnect)
local _backoff_attempt  = 0
local _backoff_deadline = 0    -- os.clock() time when next reconnect attempt is allowed
local _initialized      = false

-- ── ID generation ────────────────────────────────────────────────────────────
-- Use pollnet.nanoid() when available, fall back to simple counter.

local _id_counter = 0
local _generate_id  -- function() → string

local function _default_generate_id()
    -- Try pollnet nanoid first (available in game)
    local ok, pollnet = pcall(require, "infra.HTTP.pollnet")
    if ok and pollnet and pollnet.nanoid then
        _generate_id = pollnet.nanoid
        return _generate_id()
    end
    -- Fallback: simple counter-based ID
...
local function dispatch(envelope)
    -- Check for request/response correlation first
    if envelope.r and _pending[envelope.r] then
        ...
        if not ok then
            log.error("service_channel: request callback error for r=%s: %s", tostring(envelope.r), tostring(err))
        end
        return  -- r-field responses bypass topic handlers
    end
    ...
    if handlers then
        for _, fn in ipairs(handlers) do
            ...
            if not ok then
                log.error("service_channel: handler error for topic %s: %s", envelope.t, tostring(err))
            end
        end
    end
end
...
local function drain_messages()
    ...
    for _ = 1, MAX_MESSAGES_PER_TICK do
        ...
        if envelope then
            ...
            log.warn("service_channel: malformed message: %s", tostring(err))
        end
    end
end
...
--- Initialize the channel with a target URL.
-- Resets all state. Call tick() to begin connecting.
-- @param url  string  WebSocket URL (e.g. "ws://localhost:5557/ws?token=abc")
function M.init(url)
    if _handle then
...
-- @param topic    string  Message topic (e.g. "game.event")
...
-- Responses with `r` bypass topic handlers.
...
-- Typically used to resend config.sync.
...
--- Inject a custom ID generator (for deterministic tests).
-- @param fn  function() → string
function M._set_id_generator(fn)
    _generate_id = fn
end

--- Get the current queue length (for tests).
function M._queue_size()
    return #_queue
end

--- Get the pending request count (for tests).
function M._pending_count()
    local count = 0
    for _ in pairs(_pending) do count = count + 1 end
    return count
end
